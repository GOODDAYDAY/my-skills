...
```mermaid
flowchart LR
    A[Source code] --> B[Analyze: read-only scan]
    B --> C[Present findings to user]
    C --> D[User approves items]
    D --> E[Apply approved changes only]
...
```mermaid
flowchart LR
    A[Proposed change] --> B{Purely structural?}
    B -- Yes, 100% certain --> C[Apply change]
    B -- Uncertain --> D[Skip — list as suggestion only]
    C --> E[Verify: same results\nbefore and after]
```
**Figure 2.1 — Core principle: structural changes only**
...
flowchart LR
    ...
    B -- No --> C[Prompt user:\ncomplete coding stage first]
    B -- Yes --> D[Load documents\nand proceed]
```
**Figure 3.1 — Prerequisites check**
...
flowchart TD
    ...
    B --> C[Step 2: Analyze — read-only scan of all code]
    C --> D[Step 3: Present findings report to user]
    D --> E{User approves items?}
    E -- None approved --> F[No changes applied]
    E -- Some approved --> G[Step 4: Apply approved changes]
    G --> H[4.1 Remove unused code]
    G --> I[4.2 Extract common logic]
    G --> J[4.3 Improve module structure]
    G --> K[4.4 Clean up comments]
    H & I & J & K --> L[Step 5: Output cleanup report]
    F --> L
    L --> M[Step 6: Verify integrity — build + tests + interface check]
    M --> N{All tests pass?}
...
### 4.3 Step 3: Present Findings to User
...
### 4.4 Step 4: Apply Approved Changes
...
### 4.5 Step 5: Output Cleanup Report
